...
ExternalSSHClient::ExternalSSHClient(QObject *parent,
                                     const QString &ssh_program_path)
    : SSHClient(parent) {
  ...
  QObject::connect(ssh_process, SIGNAL(stateChanged(QProcess::ProcessState)),
                   SLOT(from_process_state_change(QProcess::ProcessState)));
  QObject::connect(ssh_process, SIGNAL(error(QProcess::ProcessError)),
                   SLOT(from_process_error(QProcess::ProcessError)));
  QObject::connect(ssh_process, SIGNAL(started()),
                   SLOT(from_process_started()));
  QObject::connect(ssh_process, SIGNAL(finished(int)),
                   SLOT(from_process_finished(int)));
  QObject::connect(ssh_process, SIGNAL(readyReadStandardOutput()),
                   SLOT(from_process_ready_read()));
}

ExternalSSHClient::~ExternalSSHClient() { delete temp_known_hosts_file; }
...
bool ExternalSSHClient::connect(const QString &host, quint16 port,
                                const QString &user, const QString &command) {
  if (ssh_program_path.isEmpty())
    return false;
  ...
  if (server_alive_interval >= 0)
    ssh_args << "-o"
             << QString("ServerAliveInterval=%1").arg(server_alive_interval);
  ssh_args << "-o"
  ...
  if (!known_hosts.isEmpty()) {
    ...
    ssh_args << "-o"
             << "UserKnownHostsFile=" + temp_known_hosts_file->fileName();
  }
  ssh_args << "-o"
  ...
  if (!identify_file.isEmpty())
    ssh_args << "-i" << identify_file;
  ssh_args << "-T";
  if (!ssh_args_extra.isEmpty())
    ssh_args << ssh_args_extra;
  if (!command.isEmpty())
    ssh_args << "--" << command;
...
bool ExternalSSHClient::atEnd() const { return ssh_process->atEnd(); }
...
bool ExternalSSHClient::isSequential() const { return true; }

void ExternalSSHClient::register_ready_read_stderr_slot(
    QObject *receiver, const char *slot, Qt::ConnectionType type) {
  // qDebug("function: ExternalSSHClient::register_ready_read_stderr_slot(%p,
  // %p<%s>, %d)", receiver, slot, slot, type);
  QObject::connect(ssh_process, SIGNAL(readyReadStandardError()), receiver,
                   slot, type);
}
...
qint64 ExternalSSHClient::writeData(const char *data, qint64 len) {
  // qDebug("function: ExternalSSHClient::writeData(%p, %lld)", data, (long long
  // int)len);
  return ssh_process->write(data, len);
}

void ExternalSSHClient::from_process_state_change(
    QProcess::ProcessState proc_state) {
...
void ExternalSSHClient::from_process_started() { emit connected(); }
