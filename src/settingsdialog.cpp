...
SettingsDialog::SettingsDialog(QWidget *parent, QSettings *config)
  ...
  bool use_internal_ssh_library =
      config->value("UseInternalSSHLibrary", false)
          .toBool() ui->checkBox_use_internal_ssh_library->setChecked(
              use_internet_ssh_library);
#else
  ...
  if (use_internal_ssh_library) {
    ...
    bool use_separate_known_hosts =
        config->value("UseSeparateKnownHosts", false).toBool();
    ui->checkBox_use_separete_known_hosts->setChecked(use_separate_known_hosts);
    if (!use_separate_known_hosts)
      ui->textEdit_known_hosts->setEnabled(false);
  }

  ui->textEdit_known_hosts->setPlainText(
      config->value("KnownHosts").toStringList().join("\n"));
  ui->lineEdit_ssh_program->setText(
  ...
  if (!font.isNull())
    ui->fontComboBox->setCurrentFont(font.value<QFont>());
#else
  ...
  ui->combo_box_language->addItem(QString("English (%1)").arg(tr("built-in")),
                                  "en");
  ...
  if (!language.isNull()) {
    ...
    if (index > 0)
      ui->combo_box_language->setCurrentIndex(index);
  }
  ...
  if (!style.isEmpty()) {
    ...
    if (index != -1)
      ui->combo_box_style->setCurrentIndex(index);
  }
  ...
  if (show_tray_icon) {
    ui->checkBox_minimize_to_tray->setChecked(
        config->value("MinimizeToTray", false).toBool());
  } else {
  ...
  ui->checkBox_tray_popup->setChecked(
      config->value("UseTrayPopup", true).toBool());
  ui->checkBox_tray_popup->setEnabled(show_tray_icon);
  ui->checkBox_window_alert_notification->setChecked(
      config->value("UseWindowAlert", false).toBool());
  ui->checkBox_sound_notification->setChecked(
      config->value("UseSound", true).toBool());
  config->endGroup();

  bool override_default_jpeg_quality =
      config->value("OverrideDefaultJPEGQuality", false).toBool();
  ui->checkBox_use_custom_jpeg_quality->setChecked(
      override_default_jpeg_quality);
  if (override_default_jpeg_quality) {
    ui->spinBox_jpeg_quality->setValue(
        config->value("JPEGQuality", 90).toInt());
  } else {
  ...
  if (d.exec()) {
    ...
    if (!QFileInfo(path).isExecutable()) {
      QMessageBox::warning(this, tr("Warning"),
                           tr("File %1 is not executable").arg(path));
    }
...
void SettingsDialog::save_settings() {
#ifdef HAVE_OPENSSH_LIBRARY
  bool use_internal_ssh_library =
      ui->checkBox_use_internal_ssh_library->isChecked();
  config->setValue("UseInternalSSHLibrary", use_internal_ssh_library);
#else
  bool use_internal_ssh_library = false;
#endif
  if (!use_internal_ssh_library)
    config->setValue("UseSeparateKnownHosts",
                     ui->checkBox_use_separete_known_hosts->isChecked());
  config->setValue("KnownHosts",
                   ui->textEdit_known_hosts->toPlainText().split('\n'));
  ...
  config->endGroup();
  config->setValue("Language", ui->combo_box_language->itemData(
                                   ui->combo_box_language->currentIndex()));
  ...
  if (show_tray_icon)
    config->setValue("MinimizeToTray",
                     ui->checkBox_minimize_to_tray->isChecked());
  config->beginGroup("Notification");
  if (show_tray_icon)
    config->setValue("UseTrayPopup", ui->checkBox_tray_popup->isChecked());
  config->setValue("UseWindowAlert",
                   ui->checkBox_window_alert_notification->isChecked());
  config->setValue("UseSound", ui->checkBox_sound_notification->isChecked());
  config->endGroup();

  bool override_default_jpeg_quality =
      ui->checkBox_use_custom_jpeg_quality->isChecked();
