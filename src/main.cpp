...
static void print_usage(const char *name) {
  fprintf(
      stderr,
      "Usage: %s [<options>] [<host>|ssh://[<user>@]<host>[:<port>][/]]\n"
      "Options:\n"
...
#ifdef Q_OS_MAC
  directories << QApplication::applicationDirPath() +
...
int main(int argc, char *argv[]) {
  ...
  static option long_options[] = {{"port", 1, NULL, 'p'},
  ...
  if (style)
    QApplication::setStyle(QString::fromLocal8Bit(style));
  ...
  if (!style) {
    ...
    if (!style.isEmpty() &&
        QString::compare(style, QObject::tr("Default"), Qt::CaseInsensitive)) {
      a.setStyle(style);
    }
  }
  if (argc - optind > 1) {
  ...
  if (language.isEmpty()) {
    ...
    if (language) {
      ...
      foreach (const QString &l, languages) {
        if (l == QString("en_US") || load_messages_translation(l))
          break;
      }
    } else {
      language = getenv("LANG");
      if (language)
        load_messages_translation(QString(language));
    }
