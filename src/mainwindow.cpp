...
MainWindow::MainWindow(QWidget *parent, QSettings *config, const QString &host,
                       quint16 port, const QString &identify_file)
  ...
  ui->action_use_html_for_sending_messages->setChecked(
      config->value("UseHTMLForSendingMessages", false).toBool());
  ...
  if (show_user_list) {
    ...
    if (!v.isNull())
      ui->dockWidget_online_list->resize(v.toSize());
  }
  {
  ...
  if (config->value("WindowMaximized", false).toBool())
    showMaximized();
  ...
  if (!log_dir->mkpath("images")) {
    ...
    QMessageBox::warning(
        this, QString(),
        tr("Failed to create image cache directory '%1'").arg(log_dir->path()));
  }
  ...
  if (!message_log->open(log_dir->filePath("messages.db"))) {
    ...
    QMessageBox::warning(
        this, QString(),
        tr("Failed to open message log database; messages won't be logged"));
  }
  connect(
      ui->chat_area->verticalScrollBar(), SIGNAL(valueChanged(int)),
      SLOT(reset_unread_message_count_from_chat_area_vertical_scroll_bar(int)));
...
void MainWindow::set_ssh_user(const QString &user) { ssh_user = user; }
  ...
  if (focusWidget() == ui->textEdit_message_to_send) {
    ...
    if (show_tip)
      ui->statusbar->showMessage(
          tr("Press %1 to send")
              .arg(send_message_on_enter ? tr("Enter") : tr("Ctrl-Enter")),
          2000);
  }
}
...
bool MainWindow::eventFilter(QObject *o, QEvent *e) {
  ...
  if (o != ui->textEdit_message_to_send)
...
void MainWindow::print_image(const QByteArray &data,
  ...
  */
  image_file_name =
      QCryptographicHash::hash(data, QCryptographicHash::Sha1).toHex() + ".jpg";
  image_file.setFileName(image_cache_dir->filePath(image_file_name));
  if (image_file.exists()) {
    ui->statusbar->showMessage(
        tr("Image file '%1' is already exist, skipping write")
            .arg(image_file_name),
        10000);
  } else {
    if (!image_file.open(QIODevice::WriteOnly)) {
      ui->chat_area->appendPlainText(tr("[Failed to save image to disk, %1]")
...
#endif
  chat_area_scroll_bar->setValue(should_scroll ? chat_area_scroll_bar->maximum()
                                               : current_scroll);
  ui->chat_area->horizontalScrollBar()->setValue(0);
  if (!my_user_name.isEmpty() && msg_from != my_user_name &&
      (!isActiveWindow() || !should_scroll)) {
    unread_message_count++;
    update_window_title();
    config->beginGroup("Notification");
    if (config->value("UseWindowAlert", false).toBool())
      QApplication::alert(this);
    config->endGroup();
  }
  message_log->append_message(
      dt, msg_from, msg_to, msg_type,
...
void MainWindow::send_message(const QString &to_user, quint8 message_type,
  ...
  if (!ssh_client->is_connected()) {
    ui->statusbar->showMessage(
        tr("Cannot send message, server is not connected"));
    return;
  }
...
void MainWindow::send_image(const QImage &image) {
  ...
  if (!image.save(&buffer, "JPEG", quality)) {
    QMessageBox::critical(this, QString(),
                          tr("Cannot export the image as JPEG"));
    return;
  }
  buffer.close();
...
void MainWindow::add_user_item(const QString &user_name,
                               QList<UserIdAndHostName> *logins) {
  QListWidgetItem *item;
  QList<QListWidgetItem *> orig_items = ui->listWidget_online_users->findItems(
      user_name, Qt::MatchFixedString | Qt::MatchCaseSensitive);
  ...
  if (!exists)
    ui->listWidget_online_users->addItem(item);
}

void MainWindow::remove_offline_user_items(const QSet<QString> &keep_set) {
  QList<QListWidgetItem *> items =
      ui->listWidget_online_users->findItems("*", Qt::MatchWildcard);
  // qDebug() << items;
...
void MainWindow::update_user_state(const QString &user, quint8 state) {
  ...
  if (state) {
    ...
    QList<QListWidgetItem *> items = ui->listWidget_online_users->findItems(
        user, Qt::MatchFixedString | Qt::MatchCaseSensitive);
    // qDebug() << items;
    if (items.isEmpty())
      return;
...
void MainWindow::print_error(quint32 error_code, const QString &error_message) {
  qDebug("MainWindow::print_error(%u, const QString &)",
...
void MainWindow::ssh_state_change(SSHClient::SSHState state) {
  ...
  case SSHClient::AUTHENTICATING:
    if (use_internal_ssh_library)
      ui->statusbar->showMessage(tr("Authenticating"));
    break;
  ...
  while (ssh_client->bytesAvailable() > 0) {
    ...
    case SSHOUT_API_PASS:
      ...
      if (version != 1) {
        ...
        ui->chat_area->appendPlainText(
            tr("Version mismatch (%1 != 1)").arg(version));
        need_reconnect = false;
        ssh_client->disconnect();
        return;
      }
      if (data.length() > 1 + 6 + 2) {
        ...
        if (1 + 6 + 2 + 1 + user_name_len > data.length()) {
          qWarning(
              "malformed SSHOUT_API_PASS packet: user_name_len %hhu too large",
              user_name_len);
          ssh_client->disconnect();
          return;
        }
        my_user_name =
            QString::fromUtf8(data.mid(1 + 6 + 2 + 1, user_name_len));
    ...
    case SSHOUT_API_RECEIVE_MESSAGE:
      // qDebug("SSHOUT_API_RECEIVE_MESSAGE received");
      {
        ...
        if (1 + 8 + 1 + from_user_len + 1 + 1 + 4 > data.length()) {
          qWarning("malformed SSHOUT_API_RECEIVE_MESSAGE packet: from_user_len "
                   "%hhu too large",
                   from_user_len);
          ssh_client->disconnect();
          return;
        ...
        quint8 to_user_len;
        stream >> to_user_len;
        if (1 + 8 + 1 + from_user_len + 1 + to_user_len + 1 + 4 >
            data.length()) {
          qWarning("malformed SSHOUT_API_RECEIVE_MESSAGE packet: to_user_len "
                   "%hhu too large",
                   to_user_len);
          ssh_client->disconnect();
          return;
        ...
        if (1 + 8 + 1 + (int)from_user_len + 1 + (int)to_user_len + 1 + 4 +
                (int)msg_len >
            data.length()) {
          qWarning("malformed SSHOUT_API_RECEIVE_MESSAGE packet: msg_len %hhu "
                   "too large",
                   msg_len);
          // ssh_client->disconnect();
          return;
        ...
        print_message(
            ...
            QString::fromUtf8(to_user, to_user_len), msg_type,
            data.mid(1 + 8 + 1 + from_user_len + 1 + to_user_len + 1 + 4,
                     msg_len));
      }
      break;
    case SSHOUT_API_USER_STATE_CHANGE:
      // qDebug("SSHOUT_API_USER_STATE_CHANGE received");
      {
        ...
        if (1 + 1 + 1 + (int)user_name_len > data.length()) {
          qWarning("malformed SSHOUT_API_USER_STATE_CHANGE packet: "
                   "user_name_len %hhu too large",
                   user_name_len);
          ssh_client->disconnect();
          return;
        }
    ...
    case SSHOUT_API_ERROR:
      // qDebug("SSHOUT_API_ERROR received");
      {
        ...
        if (1 + 4 + 4 + (int)error_msg_len > data.length()) {
          qWarning(
              "malformed SSHOUT_API_ERROR packet: error_msg_len %hhu too large",
              error_msg_len);
          ssh_client->disconnect();
          return;
        }
    ...
    case SSHOUT_API_MOTD:
      ...
      if (length > (unsigned int)data.length() - 1 - 4) {
        qWarning("malformed packet: member size %u out of packet size %d",
                 length, data.length());
        ssh_client->disconnect();
        return;
      }
...
void MainWindow::set_send_message_on_enter(bool v) {
  // qDebug("slot: MainWindow::set_send_message_on_enter(%s)", v ? "true" :
  // "false");
...
void MainWindow::send_image() {
  ...
  if (d.exec()) {
    ...
    if (image.isNull()) {
      QMessageBox::critical(this, QString(),
                            tr("Cannot load file '%1' as an image").arg(path));
      return;
    }
    send_image(image);
  }
...
void MainWindow::show_about() {
  ...
  d.setText("<h3>Secure Shout Host Oriented Unified Talk</h3>Copyright 2020 "
            ...
            "<br/>" +
            tr("There is NO warranty; not even for MERCHANTABILITY or FITNESS "
               "FOR A PARTICULAR PURPOSE."));
  d.exec();
}

void MainWindow::show_chat_area_context_menu(const QPoint &p) {
  // qDebug("slot: MainWindow::show_chat_area_context_menu(QPoint(%d, %d))",
  // p.x(), p.y()); QMenu *menu = ui->chat_area->createStandardContextMenu(p);
  ...
  if (cursor.block().text() == QString(QChar(0xfffc))) {
    ...
    // SLOT(open_image_from_chat_area())); pos_in_chat_area = p;
  ...
  menu->addAction(tr("&Copy") + "	" +
                  ...
                  ui->chat_area, SLOT(copy()))
      ->setEnabled(ui->chat_area->textCursor().hasSelection());
  /*
          if(ui->chat_area->interactionFlags() & Qt::LinksAccessibleByKeyboard
     || (ui->chat_area->interactionFlags() & Qt::LinksAccessibleByMouse)) {
                  menu->addAction(tr("Copy &Link Location"), this,
     SLOT(copy_link_from_chat_area()))->setEnabled(is_url(ui->chat_area,
     cursor.position()));
          }
  */
  menu->addAction(tr("Select All") + "	" +
                      QKeySequence(QKeySequence::SelectAll).toString(),
                  ui->chat_area, SLOT(selectAll()))
      ->setEnabled(!ui->chat_area->document()->isEmpty());
  ...
  if (triggered_action) {
    if (triggered_action == open_image_action) {
      ...
      QUrl url(cursor.charFormat().toImageFormat().name());
      QVariant v = ui->chat_area->document()->resource(
...
void MainWindow::update_window_title() {
  if (unread_message_count)
    setWindowTitle(
        QString("[%1] %2 - SSHOUT").arg(unread_message_count).arg(host));
  else
    setWindowTitle(QString("%1 - SSHOUT").arg(host));
}
...
void MainWindow::changeEvent(QEvent *e) {
  ...
  if (!isActiveWindow())
    return;
  QScrollBar *chat_area_scroll_bar = ui->chat_area->verticalScrollBar();
  if (chat_area_scroll_bar->value() < chat_area_scroll_bar->maximum())
    return;
  reset_unread_message_count();
}
...
void MainWindow::dropEvent(QDropEvent *e) {
  ...
  // qDebug() << data->hasHtml() << data->hasImage() << data->hasUrls();
  if (data->hasImage())
    send_image(qvariant_cast<QImage>(data->imageData()));
  else if (data->hasUrls()) {
    QUrl url = data->urls()[0];
    if (url.scheme() != QString("file")) {
      ui->statusbar->showMessage(
          tr("Can't load '%1' as image: only local files are supported")
              .arg(url.toString()),
          10000);
      return;
    }
    ...
    if (image.isNull()) {
      QString error_string =
          tr("Cannot load file '%1' as an image").arg(url.toString());
