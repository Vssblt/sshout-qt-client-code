...
class SSHClient : public QIODevice {
  Q_OBJECT
public:
  enum SSHState { DISCONNECTED, CONNECTIING, AUTHENTICATING, AUTHENTICATED };
  enum SSHError { SSH_UNKNOWN_ERROR };

  SSHClient(QObject * = NULL);
  virtual bool connect(const QString &, quint16, const QString &,
