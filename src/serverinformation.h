...
static inline QDataStream &operator<<(QDataStream &out,
                                      const ServerInformation &info) {
  return out << info.host << info.port << info.identify_file;
}

static inline QDataStream &operator>>(QDataStream &in,
...
static inline bool operator==(const ServerInformation &v1,
                              const ServerInformation &v2) {
  return v1.host == v2.host && v1.port == v2.port &&
         v1.identify_file == v2.identify_file;
}
